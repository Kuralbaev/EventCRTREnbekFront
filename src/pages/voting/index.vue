      ...
      >
        <div class="flex items-center">
          ...
          >
            ...
            <template v-if="!check_commission">
              <a-form-item
                :label="$t('application.employee.form.region')"
                name="region"
            ...
            <template v-else>
              <a-form-item
                :label="$t('application.employee.form.work_place')"
                name="work_place"
                class="w-full input"
              >
                <a-input
                  v-model:value="formState.work_place"
                  class="h-[40px] md:h-[56px] w-full !rounded-[24px] !border-[#E0E5EA] disabled:!text-black !bg-[#F0F2F4] !text-[14px] md:!text-[20px] !font-medium !px-6 !cursor-default"
                />
              </a-form-item>
            </template>
        ...
        <template v-if="check_commission">
          ...
          <div class="flex gap-3">
            ...
            >
              {{ votingList?.length }} проголосовали
            </p>
            <p
              class="text-[16px] font-medium bg-yellow-500 text-white rounded-[12px] p-2 px-3 w-max"
            >
              {{ filterUsers?.length - votingList?.length }} не проголосовали
            </p>
          </div>
        </template>
      </div>
...
const { check_commission, votingList, voting_ids, user, users_nomination } =
  storeToRefs(store);
...
onMounted(async () => {
  await store.fetchUsersNomination();
  await store.fetchVoting();
  loading.value = false;
});

const filterUsers = computed(() => {
  return users_nomination.value
    .filter((user) =>
      !!formState.fio
    ...
    .filter((user) =>
      !!formState.nomination
        ? formState.nomination === "Проект"
          ? !user.nomination
          : user.nomination === formState.nomination
        : true
    )
    .filter((user) =>
      !!formState.work_place
